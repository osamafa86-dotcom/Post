...
@section('title')
    {{config('system.site_name') . ' - '}}{{ __('messages.draft_posts.draft') }}
@endsection
@section('style')
<style>
    .loading-overlay {
...
<div>
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-muted">{{ __('messages.draft_posts.draft') }}</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
            ...
            <div class="card card-flush">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1"
                                        ...
                                        <li><a class="dropdown-item" data-bs-toggle="modal"
                                               data-bs-target="#bulkpostChangeStatus">
                                                {{ __('messages.draft_posts.publish') }}
                ...
                <div class="card-body pt-0 table-responsive">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                        ...
                        @foreach($this->draft_posts as $key =>  $post)
                            <tr>
                                @can('posts_delete')
                                ...
                                @if(isset($columnVisibility['actions']) && $columnVisibility['actions'])
                                    @canany(['posts_show','posts_edit','posts_delete'])
                                        <td class="text-start ">
                                            {{-- Keep your existing actions dropdown here --}}
                                            <div class="dropdown">
            ...
            <div class="modal-content">
                <div class="modal-header bg-danger">
                    <h5 class="modal-title text-white" id="staticBackdropLabel">
                        {{ __('messages.posts.delete_post') }} </h5>
    ...
    <div class="modal fade" id="postDeleteSelected" data-bs-backdrop="static" data-bs-keyboard="false"
         tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-danger">
                    <h5 class="modal-title text-white" id="staticBackdropLabel">
                        {{ __('messages.posts.delete_posts') }} </h5>
                    <button type="button" class="btn-close text-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h5 class="modal-title text-white" id="staticBackdropLabel">
                        {{ __('messages.posts.change_status') }}</h5>
                    <button type="button" class="btn-close text-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <h4>{{ __('messages.posts.confirm_Publish_status') }} </h4>
                    <h6>{{ __('messages.posts.change_status_message') }} :
                        {{ $Post->title ?? __('messages.not_available') }}</h6>
                    <div class="modal-footer p-0">
                        <button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">{{ __('messages.cancel') }}</button>
                        <button wire:click="confirmChangeStatus" data-bs-dismiss="modal"
                                class="btn btn-success">{{ __('messages.confirm') }}
                        </button>
                    </div>
                </div>
            </div>
    ...
    <!--Change Status Modal -->
    <div class="modal fade" id="bulkpostChangeStatus" data-bs-backdrop="static" data-bs-keyboard="false"
         tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h5 class="modal-title text-white" id="staticBackdropLabel">
                        {{ __('messages.posts.change_status_selected') }} </h5>
                    <button type="button" class="btn-close text-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <h4>{{ __('messages.posts.confirm_publish_status_selected') }}</h4>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                        ...
                        <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('order_number')">
                                {{ __('messages.draft_posts.order') }}
                            </th>
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('id')">
                                {{ __('messages.draft_posts.post_id') }}
                            </th>
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('title')">
                                {{ __('messages.draft_posts.title') }}
                            </th>
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('category_id')">
                                {{ __('messages.draft_posts.category') }}
                            </th>
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('views')">
                                {{ __('messages.draft_posts.views') }}
                            </th>
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('user_id')">
                                {{ __('messages.draft_posts.supervisor') }}
                            </th>
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('publish_date')">
                                {{ __('messages.draft_posts.publish_time') }}
                            </th>
                            <th class="text-start min-w-70px th-btn" wire:click="sortBy('updates')">
                                {{ __('messages.draft_posts.updates') }}
                            </th>
                            @canany(['posts_show', 'posts_edit', 'posts_delete'])
                                <th class="text-start min-w-70px">{{ __('messages.actions') }}</th>
                            @endcanany
                        ...
                        @foreach ($this->draft_posts as $key => $post)
                            <tr>
                                <td class="text-start ">
                                    {{$post->order_number ?? __('messages.none')}}
                                </td>
                                <td class="text-start ">
                                    {{$post->id ?? __('messages.none')}}
                                </td>
                                <td class="text-start ">
                                    {{$post->title ?? __('messages.none')}}
                                </td>
                                <td class="text-start ">
                                    {{$post?->category->category_title ?? __('messages.none')}}
                                </td>
                                <td class="text-start ">
                                    {{$post->views ?? __('messages.none')}}
                                </td>
                                <td class="text-start ">
                                    {{$post?->user->full_name ?? __('messages.none')}}
                                </td>
                                <td class="text-start ">
                                    {{Carbon::parse($post->publish_date)->format('H:i Y-m-d') ?? __('messages.none')}}
                                </td>
                                <td class="text-start ">
                                    {{$post->updates ?? __('messages.none')}}
                                </td>
                                @canany(['posts_show', 'posts_edit', 'posts_delete'])
                                    <td class="text-start ">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1"
                                                ...
                                                @can('posts_edit')
                                                    <li><a class="dropdown-item text-success"
                                                           wire:click="changeStatus({{$post}})"
                                                           data-bs-toggle="modal" data-bs-target="#postChangeStatus">
                                                            {{ __('messages.publish') }}
                                                            <i class="bi bi-check-circle text-success"></i>
                                                        </a>
                                                    </li>
