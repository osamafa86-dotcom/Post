...
@section('title')
    {{config('system.site_name') . ' - '}}{{ __('messages.video_tracks.videos') }}
@endsection
@section('style')
    <style>
        .th-btn {
...
<div>
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">
                    {{ __('messages.video_tracks.videos') }}
                </h1>
                <!--end::Title-->
                <!--begin::Breadcrumb-->
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-muted">{{ __('messages.video_tracks.videos') }}</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            <div class="card card-flush">
                ...
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        ...
                        <div class="d-flex align-items-center position-relative my-1 gap-2 flex-wrap">
                            <div class="input-group">
                                ...
                                <input type="text" wire:model.live="search"
                                       ...
                                       placeholder="{{ __('messages.video_tracks.search_video') }}"/>
                            </div>
                    ...
                    <!--end::Card title-->
                    <!--begin::Card toolbar-->
                    @can('video_tracks_create')
                        <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                            <!--begin::Add product-->
                            <a wire:click="addNew"
                               class="btn btn-primary">{{ __('messages.video_tracks.add_video') }}</a>
                            <!--end::Add product-->
                        </div>
                    @endcan
                    <!--end::Card toolbar-->
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                        ...
                        <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                            <th class="text-start min-w-70px th-btn"
                                wire:click="sortBy('id')">{{ __('messages.video_tracks.video_id') }}
                            </th>
                            <th class="text-start min-w-70px th-btn"
                                wire:click="sortBy('title')">{{ __('messages.video_tracks.video_title') }}
                            </th>
                            <th class="text-start min-w-70px th-btn"
                                wire:click="sortBy('description')">{{ __('messages.video_tracks.video_description') }}
                            </th>
                            @canany(['video_tracks_edit','video_tracks_delete'])
                        ...
                        @foreach($this->tracks as $key => $track)
                            <tr>
                                ...
                                <td class="text-start ">
                                    {{$track->title ?? __('messages.not_available')}}
                                </td>
                                <td class="text-start  text-truncate">
                                    {{Str::limit($track->description,50) ?? __('messages.not_available')}}
                                </td>
                                @canany(['video_tracks_edit','video_tracks_delete'])
                                    <td class="text-start ">
                                        <div class="dropdown">
    ...
    <!--end::Content-->
        ...
        @include('components.modals.video_track_modals')
                ...
                Swal.fire(
                    {
                        ...
                        text: '{{ __('messages.alert_message.video_saved') }}',
                        icon: 'success',
        ...
        document.addEventListener('DOMContentLoaded', function () {
            document.body.addEventListener('hide_track_delete', function () {
                Swal.fire(
                    {
                        title: '{{ __('messages.alert_message.success') }}',
                        text: '{{ __('messages.alert_message.video_deleted') }}',
                        icon: 'success',
                        confirmButtonText: '{{ __('messages.alert_message.done') }}',
                        customClass: {
                            confirmButton: 'btn btn-success'
