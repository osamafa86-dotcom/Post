...
@section('title')
    {{config('system.site_name')  ? config('system.site_name') : 'فلسطين بوست' }} | {{$state['title'] ?? 'تفاصيل الخبر'}}
@endsection

@section('description')
    {{ strip_tags($state['description']) }}
@endsection

@section('og_image')
    @if($state['image_name'])
        <meta property="og:image" content="{{ file_url($state['image_name']) }}"/>
    @endif
@endsection

@section('twitter_image')
    @if($state['image_name'])
        <meta name="twitter:image" content="{{ file_url($state['image_name']) }}"/>
    @endif
@endsection
...
<div>
    ...
    <div class="article-page">
        <div class="container">
            @if(isset($state['is_preview']) && $state['is_preview'])
                @if(!$state['is_available'])
                    <div class="alert alert-warning mb-4">
                        <i class="bi bi-clock"></i>
                        {{ __('messages.posts.not_available_yet') }}
                        @if(isset($state['available_date']))
                            <br><small class="mt-1 d-block">
                                {{ __('messages.posts.available_from') }}:
                                {{ \Carbon\Carbon::parse($state['available_date'])->format('Y-m-d H:i') }}
                            </small>
                        @endif
                    </div>
                @else
                    <div class="alert alert-info mb-4">
                        <i class="bi bi-eye"></i>
                        {{ __('messages.posts.preview_mode') }}
                    </div>
                @endif
            @endif
            <div class="row">
                <div class="col-md-8 offset-md-2 ">
                    <div class="article-content">
                        <div class="img-article">
                            @if(isset($state['image_name']) && $state['image_name'])
                        ...
                        <header>
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="date-article d-flex align-items-center gap-2">
                                    ...
                                    {{Carbon::parse($state['publish_date'])->isoFormat('D MMMM YYYY')}}
                                </div>
                            ...
                            @php
                                $authors=\App\Models\Participant::whereIn('id',$state['author_id'])->with(['files.file','participant_social_media'])->get();
                            @endphp
                            @if($authors->isNotEmpty())
                                @forelse($authors as $author)
                                    <div class="article-details  d-flex align-items-center justify-content-between ">
                                        <a href="{{route('main.palestine_post.writer' , ['author_id' => $author?->id , 'author_name' => $author?->name])}}">

                                            <div class="author-details d-flex align-items-center gap-2">
{{--                                                <div class="avtar-author ">--}}
{{--                                                    <img--}}
{{--                                                        src="{{file_url($author->files?->first()?->file?->path)}}"--}}
{{--                                                        alt="">--}}
{{--                                                </div>--}}
                                                <div class="author-text flex-column  align-items-start gap-1">
                                                    <h4 class="name">   {{$author?->name}} </h4>
                                                    <p class="title-job">    {{$author?->work}}</p>
                                                </div>

                                            </div>
                                        </a>
                                        <ul class="social-links info-card ">
                                            @foreach($author->participant_social_media as $social_media)
                                ...
                                @php
                                    ...
                                    if (!empty($state['tags'])) {
                                        if (is_array($state['tags'])) {
                                            $tags = $state['tags'];
                                        } elseif (is_string($state['tags'])) {
                                            $tags = explode(',', $state['tags']);
                                        }
                                    }
