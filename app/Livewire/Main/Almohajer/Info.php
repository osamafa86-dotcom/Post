...
class Info extends Component
{
    use WithFileUploads;
    #[Computed]
    public function data_files()
    {
        return ImmigrantData::query()->with(['files'])
            ->where('type', DataPageEnum::FILE->value)
    ...
    {
        return ImmigrantData::query()->with(['files'])
            ->where('type', DataPageEnum::CODE->value)
            ->orderByDesc('created_at')
            ->take(5)
            ->get();
    }
    ...
    {
        return ImmigrantData::query()->with(['files'])
            ->where('type', DataPageEnum::OPPORTUNITY->value)
            ->orderByDesc('created_at')
            ->take(5)
            ->get();
    }
    ...
    {
        return view('livewire.main.almohajer.info');
    }
}
