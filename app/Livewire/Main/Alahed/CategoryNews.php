...
class CategoryNews extends Component
{
    ...
    {
        return view('livewire.main.alahed.category-news');
    }
    ...
    public function mount(string $category_title): void
    {
        ...
        $this->most_viewed_posts = Post::with(['category.relationable', 'files.file'])
            ->withSum('views as views_sum', 'views_number')
        ->orderByRaw('COALESCE(views_sum, 0) DESC')
        ->orderByDesc('publish_date')
            ->take(5)
            ->get();
        $this->latest_posts = Post::with(['category.relationable', 'files.file'])->orderBy('publish_date', 'desc')->take(5)->get();
        $this->article_posts = Post::with(['category.relationable', 'files.file'])->whereHas('category',function ($q){$q->where('category_title', 'مقالات');})->orderBy('publish_date', 'desc')->take(5)->get();
        $this->loadPosts();
    }
    ...
    public function loadPosts(): void
    {
        ...
        $this->posts = $query->take($this->posts_number)->get();
